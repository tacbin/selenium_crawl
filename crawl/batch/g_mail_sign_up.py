...
class GEmailSignUpCrawl(CommonCrawl):
    def __init__(self):
        super().__init__()
        self.result_map = {}
        self.use_proxy = True
    ...
    def before_crawl(self, args, browser: WebDriver) -> WebDriver:
        self.result_map = {}
        self.urls = [
            "https://accounts.google.com/lifecycle/steps/signup/name?dsh=S269092966:1700392587359268&flowEntry=SignUp&flowName=GlifWebSignIn",
        ]
        for url in self.urls:
            self.result_map[url] = []

        self.file_location = 'GEmailSignUpCrawl'
        self.is_save_img = False
        self.mode = 1
        return browser

    def parse(self, browser: WebDriver):
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'GEmailSignUpCrawlstart crawl..',
              browser.current_url)
        time.sleep(10)
        # 用户信息填写
        GEmailSignUpCrawl.user_info(browser)
        time.sleep(10)

        browser.find_elements(By.XPATH, "//div[@class='enBDyd ']")[0].click()
        time.sleep(5)
        browser.find_elements(By.XPATH, '//button')[0].click()
        time.sleep(10)
        browser.find_elements(By.XPATH, "//input[@type='password']")[0].send_keys("Miemiemie")
        time.sleep(5)

        browser.find_elements(By.XPATH, "//input[@type='password']")[1].send_keys("Miemiemie")
        time.sleep(10)
        ...
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'GEmailSignUpCrawlend crawl..',
              browser.current_url)
    ...
    def custom_send(self):
        for url in self.result_map:
            print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'GEmailSignUpCrawlstart custom_send..', url)
