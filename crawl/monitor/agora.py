...
class AgoraCrawl(CommonCrawl):
    ...
    def parse(self, browser: WebDriver):
        ...
        for card in cards:
            ...
            url = url.replace('\n', '')
            url = "https://www.100Agora.live" + url
    ...
    def custom_send(self):
        for url in self.result_map:
            ...
            for data in self.result_map[url]:
                txt = '【100Agora.live】发文章啦\n' \
                      '标题:%s\n' \
                      '时间:%s\n' \
                      '链接:%s' % (data.title, data.date, data.url)
                QQRobot.send_group_Agorag(461936572, [miraicle.Plain(txt)])
                QQRobot.send_group_Agorag(963961013, [miraicle.Plain(txt)])
