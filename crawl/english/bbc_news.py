...
from common.constants import Week, FuDuJiGroup, Day
...
class BBCNewsCrawler(CommonCrawl):
    def __init__(self):
        super().__init__()
        self.sub_dir = time.strftime('%Y-%m-%d', time.localtime())
        self.result_map = {}
        last_time = CommonInstance.Redis_client.get('BBCNewsCrawler')
        if last_time is None:
            last_time = get_current_time()
        else:
            last_time = int(last_time)
        if get_current_time() - Day < last_time:
            self.is_run = True
    ...
    def before_crawl(self, args, browser: WebDriver) -> WebDriver:
        ...
        self.file_location = 'BBCNewsCrawler  '
        self.is_save_img = False
        self.mode = 1

        new_dir = './files/bbc_news/%s' % self.sub_dir
    ...
    def parse(self, browser: WebDriver):
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'BBCNewsCrawler   start crawl..',
              browser.current_url)
        ...
        tasks = selector.xpath("//div[@id='dismissible']")
        count = 1
        for task in tasks:
            sel = etree.HTML(etree.tostring(task, method='html'))

            url = sel.xpath("//a[@id='thumbnail']/@href")
            ...
            if count >= 5:
                break
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'BBCNewsCrawler   end crawl..',
              curr_url)
    ...
    def custom_send(self):
        ...
        for url in self.result_map:
            print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'BBCNewsCrawler   start custom_send..',
                  url)
            for data in self.result_map[url]:
                ...
                try:
                    ...
                    print("BBCNewsCrawler err:", e)
            for t in threads:
                t.join()
        CommonInstance.Redis_client.set('BBCNewsCrawler', get_current_time())
        QQRobot.send_group_msg(FuDuJiGroup, ["新闻 更新完毕"])
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'BBCNewsCrawler  ending..')
...
class threadControl(threading.Thread):
    ...
    def download_audio(self, url, i):
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ...
            mp3_filename = './files/bbc_news/%s/%d-%s.mp3' % (self.sub_dir, self.i, video_title)
            # Subtitle file name
            subtitle_filename = './files/bbc_news/%s/%d-%s.vtt' % (self.sub_dir, self.i, video_title)
            img_filename = './files/bbc_news/%s/%d-%s.jpg' % (self.sub_dir, self.i, video_title)
            video_filename = './files/bbc_news/%s/%d-%s.mp4' % (self.sub_dir, self.i, video_title)

            self.remove_files([mp3_filename, subtitle_filename, img_filename, video_filename])
            ...
            #  删除文件
            self.remove_files([audio_path, subtitle_path, img_path, webp_path, video_path])
