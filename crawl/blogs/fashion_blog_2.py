...
class FashionBlog2Crawl(CommonCrawl):
    def __init__(self):
        super().__init__()
        self.result_map = {}
    ...
    def before_crawl(self, args, browser: WebDriver) -> WebDriver:
        ...
        self.file_location = 'FashionBlog2Crawl'
    ...
    def parse(self, browser: WebDriver):
        ...
        for task in tasks:
            ...
            date = sel.xpath("//a//span/text()")
            date = date[0] if len(date) > 0 else ''
            date = date.replace('\n', '').strip()

            desc = sel.xpath('//div[@class="entry-content"]/text()')
            desc = ''.join(desc)
            desc = desc.replace('\n', '').strip()
        ...
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'FashionBlog2Crawlend crawl..', browser.current_url)

    def custom_send(self):
        for url in self.result_map:
            print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), 'FashionBlog2Crawlstart custom_send..', url)
